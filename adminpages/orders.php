...
if ( ! empty( $_REQUEST['save'] ) ) {
	...
	//update values
	if ( !empty($_REQUEST['copy']) || ! in_array( "code", $read_only_fields ) && isset( $_POST['code'] ) ) {
		$order->code = sanitize_text_field($_POST['code']);
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "user_id", $read_only_fields ) && isset( $_POST['user_id'] ) ) {
		$order->user_id = intval( $_POST['user_id'] );
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "membership_id", $read_only_fields ) && isset( $_POST['membership_id'] ) ) {
		$order->membership_id = intval( $_POST['membership_id'] );
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "billing_name", $read_only_fields ) && isset( $_POST['billing_name'] ) ) {
		$order->billing->name = stripslashes( $_POST['billing_name'] );
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "billing_street", $read_only_fields ) && isset( $_POST['billing_street'] ) ) {
		$order->billing->street = stripslashes( $_POST['billing_street'] );
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "billing_city", $read_only_fields ) && isset( $_POST['billing_city'] ) ) {
		$order->billing->city = stripslashes( $_POST['billing_city'] );
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "billing_state", $read_only_fields ) && isset( $_POST['billing_state'] ) ) {
		$order->billing->state = stripslashes( $_POST['billing_state'] );
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "billing_zip", $read_only_fields ) && isset( $_POST['billing_zip'] ) ) {
		$order->billing->zip = sanitize_text_field($_POST['billing_zip']);
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "billing_country", $read_only_fields ) && isset( $_POST['billing_country'] ) ) {
		$order->billing->country = stripslashes( $_POST['billing_country'] );
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "billing_phone", $read_only_fields ) && isset( $_POST['billing_phone'] ) ) {
		$order->billing->phone = sanitize_text_field($_POST['billing_phone']);
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "subtotal", $read_only_fields ) && isset( $_POST['subtotal'] ) ) {
		$order->subtotal = sanitize_text_field($_POST['subtotal']);
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "tax", $read_only_fields ) && isset( $_POST['tax'] ) ) {
		$order->tax = sanitize_text_field($_POST['tax']);
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "couponamount", $read_only_fields ) && isset( $_POST['couponamount'] ) ) {
		$order->couponamount = sanitize_text_field($_POST['couponamount']);
	}
	if ( !empty($_REQUEST['copy']) || ! in_array( "total", $read_only_fields ) && isset( $_POST['total'] ) ) {
		$order->total = sanitize_text_field($_POST['total']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "payment_type", $read_only_fields ) && isset( $_POST['payment_type'] ) ) {
		$order->payment_type = sanitize_text_field($_POST['payment_type']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "cardtype", $read_only_fields ) && isset( $_POST['cardtype'] ) ) {
		$order->cardtype = sanitize_text_field($_POST['cardtype']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "accountnumber", $read_only_fields ) && isset( $_POST['accountnumber'] ) ) {
		$order->accountnumber = sanitize_text_field($_POST['accountnumber']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "expirationmonth", $read_only_fields ) && isset( $_POST['expirationmonth'] ) ) {
		$order->expirationmonth = sanitize_text_field($_POST['expirationmonth']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "expirationyear", $read_only_fields ) && isset( $_POST['expirationyear'] ) ) {
		$order->expirationyear = sanitize_text_field($_POST['expirationyear']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "ExpirationDate", $read_only_fields ) && isset( $_POST['ExpirationDate'] ) ) {
		$order->ExpirationDate = $order->expirationmonth . $order->expirationyear;
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "status", $read_only_fields ) && isset( $_POST['status'] ) ) {
		$order->status = stripslashes( $_POST['status'] );
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "gateway", $read_only_fields ) && isset( $_POST['gateway'] ) ) {
		$order->gateway = sanitize_text_field($_POST['gateway']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "gateway_environment", $read_only_fields ) && isset( $_POST['gateway_environment'] ) ) {
		$order->gateway_environment = sanitize_text_field($_POST['gateway_environment']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "payment_transaction_id", $read_only_fields ) && isset( $_POST['payment_transaction_id'] ) ) {
		$order->payment_transaction_id = sanitize_text_field($_POST['payment_transaction_id']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "subscription_transaction_id", $read_only_fields ) && isset( $_POST['subscription_transaction_id'] ) ) {
		$order->subscription_transaction_id = sanitize_text_field($_POST['subscription_transaction_id']);
	}
	if ( !empty($_REQUEST['copy']) ||  ! in_array( "notes", $read_only_fields ) && isset( $_POST['notes'] ) ) {
