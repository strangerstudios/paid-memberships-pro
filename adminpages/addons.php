...
foreach ( $addons as $addon ) {
	...
	<p>
		<?php printf( __( 'Last checked on %1$s at %1$s.', 'pmpro' ), date_i18n( get_option( 'date_format' ), $addons_timestamp ), date_i18n( get_option( 'time_format' ), $addons_timestamp ) );?> &nbsp;  
	...
	<tbody id="the-list">
		...
		if ( $status == 'active' ) {
			$addons = $addons_active; } elseif ( $status == 'inactive' ) {
			$addons = $addons_inactive; } elseif ( $status == 'update' ) {
				$addons = $addons_update; } elseif ( $status == 'uninstalled' ) {
				$addons = $addons_uninstalled; } else {
					$addons = $addons_all; }
			...
			<?php
				...
				foreach ( $addons as $addon ) {
					...
					if ( file_exists( $plugin_file_abs ) ) {
						$plugin_data = get_plugin_data( $plugin_file_abs, false, true ); } else {
						$plugin_data = $addon; }
			...
			*/ ?>
					...
					if ( $addon['License'] == 'free' ) {
						_e( 'PMPro Free', 'pmpro' ); } elseif ( $addon['License'] == 'core' ) {
						_e( 'PMPro Core', 'pmpro' ); } elseif ( $addon['License'] == 'plus' ) {
							_e( 'PMPro Plus', 'pmpro' ); } elseif ( $addon['License'] == 'wordpress.org' ) {
							_e( 'WordPress.org', 'pmpro' ); } else {
								_e( 'N/A', 'pmpro' ); }
					?>
					</td>
