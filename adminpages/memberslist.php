...
if ( ! function_exists( 'current_user_can' ) || ( ! current_user_can( 'manage_options' ) && ! current_user_can( 'pmpro_memberslist' )) ) {
	...
	global $wpdb;
if ( isset( $_REQUEST['s'] ) ) {
	$s = sanitize_text_field( trim( $_REQUEST['s'] ) ); } else { 		$s = ''; }
...
?>
	...
	if ( isset( $_REQUEST['limit'] ) ) {
		$limit = intval( $_REQUEST['limit'] ); } else {
		/**
		...
		if ( $s ) {
			...
			if ( $l == 'oldmembers' ) {
				$sqlQuery .= " AND mu.status <> 'active' AND mu2.status IS NULL "; } elseif ( $l == 'expired' ) {
				$sqlQuery .= " AND mu.status = 'expired' AND mu2.status IS NULL "; } elseif ( $l == 'cancelled' ) {
					$sqlQuery .= " AND mu.status IN('cancelled', 'admin_cancelled') AND mu2.status IS NULL "; } elseif ( $l ) {
					$sqlQuery .= " AND mu.status = 'active' AND mu.membership_id = '" . esc_sql( $l ) . "' "; } else { 		$sqlQuery .= " AND mu.status = 'active' "; }

					$sqlQuery .= 'GROUP BY u.ID ';

					if ( $l == 'oldmembers' || $l == 'expired' || $l == 'cancelled' ) {
						$sqlQuery .= 'ORDER BY enddate DESC '; } else { 		$sqlQuery .= 'ORDER BY u.user_registered DESC '; }
		...
		<thead>
			<tr class="thead">
				...
				<th>
					<?php
					if ( $l == 'oldmembers' ) {
						_e( 'Ended', 'pmpro' ); } else { 							_e( 'Expires', 'pmpro' ); }
					?>
				</th>
			</tr>
		</thead>
					...
					<td class="username column-username">
						...
						<strong>
							...
							<td>
								<?php
								if ( $auser->enddate ) {
									echo apply_filters( 'pmpro_memberslist_expires_column', date_i18n( get_option( 'date_format' ), $auser->enddate ), $auser ); } else { 								echo __( apply_filters( 'pmpro_memberslist_expires_column', 'Never', $auser ), 'pmpro' ); }
								?>
							</td>
						</tr>
					<?php
