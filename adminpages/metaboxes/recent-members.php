...
function pmpro_dashboard_report_recent_members_callback() {
	...
	if ( false === ( $theusers = get_transient( 'pmpro_dashboard_report_recent_members' ) ) ) {
		...
		LIMIT 5";
		$theusers = $wpdb->get_results( $sqlQuery ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Static query; only $wpdb table names are interpolated. Joins PMPro custom tables; result is cached in a transient.
