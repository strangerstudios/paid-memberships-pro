...
if ( ! $pmpro_ready ) {
	...
	if ( ! isset( $edit ) ) {
		if ( isset( $_REQUEST['edit'] ) ) {
			$edit = $_REQUEST['edit']; } else { 			$edit = false; }
	}

	if ( empty( $msg ) ) {
		$msg = -1; }
	if ( empty( $pmpro_level_ready ) && empty( $edit ) ) {
		$msgt .= ' <a href="' . admin_url( 'admin.php?page=pmpro-membershiplevels&edit=-1' ) . '">' . __( 'Add a membership level to get started.', 'pmpro' ) . '</a>'; } elseif ( $pmpro_level_ready && ! $pmpro_pages_ready && $view != 'pmpro-pagesettings' ) {
		$msgt .= ' <a href="' . admin_url( 'admin.php?page=pmpro-pagesettings' ) . '">' . __( 'Set up the membership pages', 'pmpro' ) . '</a>.'; } elseif ( $pmpro_level_ready && $pmpro_pages_ready && ! $pmpro_gateway_ready && $view != 'pmpro-paymentsettings' ) {
...
if ( ! pmpro_checkLevelForStripeCompatibility() ) {
	...
	if ( $view == 'pmpro-membershiplevels' && ! empty( $_REQUEST['edit'] ) && $_REQUEST['edit'] > 0 ) {
		...
		$msgt .= ' ' . __( 'The levels with issues are highlighted below.', 'pmpro' ); } else { 	$msgt .= ' <a href="' . admin_url( 'admin.php?page=pmpro-membershiplevels' ) . '">' . __( 'Please edit your levels', 'pmpro' ) . '</a>.'; }
}
