<?php
	...
	if ( isset( $_REQUEST['edit'] ) ) {
		...
		// Warn about levels or discount code overrides whose set expiration date is in
		// the past (which blocks new signups) or whose stored pattern can no longer be
		// parsed (which is ignored at checkout).
		$past_expiration_items    = array();
		$invalid_expiration_items = array();
		foreach ( pmpro_getAllLevels() as $expiration_check_level ) {
			$set_expiration_date = pmpro_get_set_expiration_date( $expiration_check_level->id );
			if ( empty( $set_expiration_date ) ) {
				continue;
			}
			$expiration_check_link = '<a href="' . esc_url( add_query_arg( array( 'page' => 'pmpro-membershiplevels', 'edit' => $expiration_check_level->id ), admin_url( 'admin.php' ) ) ) . '">' . esc_html( $expiration_check_level->name ) . '</a>';
			$resolved_date = pmpro_payment_schedule_resolve_expiration_date( $set_expiration_date );
			if ( empty( $resolved_date ) ) {
				$invalid_expiration_items[] = $expiration_check_link;
			} elseif ( $resolved_date <= wp_date( 'Y-m-d' ) ) {
				$past_expiration_items[] = $expiration_check_link;
			}
		}
		$code_expiration_options = $wpdb->get_results( "SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE 'pmprosed\\_%\\_%' AND option_value <> ''" );
		foreach ( $code_expiration_options as $code_expiration_option ) {
			if ( ! preg_match( '/^pmprosed_(\d+)_(\d+)$/', $code_expiration_option->option_name, $matches ) ) {
				continue;
			}
			$expiration_check_link = '<a href="' . esc_url( add_query_arg( array( 'page' => 'pmpro-discountcodes', 'edit' => intval( $matches[2] ) ), admin_url( 'admin.php' ) ) ) . '">' . sprintf( /* translators: 1: a membership level ID, 2: a discount code ID. */ esc_html__( 'level %1$d via discount code %2$d', 'paid-memberships-pro' ), intval( $matches[1] ), intval( $matches[2] ) ) . '</a>';
			$resolved_date = pmpro_payment_schedule_resolve_expiration_date( $code_expiration_option->option_value );
			if ( empty( $resolved_date ) ) {
				$invalid_expiration_items[] = $expiration_check_link;
			} elseif ( $resolved_date <= wp_date( 'Y-m-d' ) ) {
				$past_expiration_items[] = $expiration_check_link;
		...
		if ( ! empty( $past_expiration_items ) || ! empty( $invalid_expiration_items ) ) {
			$expiration_warning_allowed_html = array(
				'strong' => array(),
				'a'      => array( 'href' => array() ),
			);
			?>
			<div class="notice notice-warning">
				<?php if ( ! empty( $past_expiration_items ) ) { ?>
					<p>
					<?php
						printf(
				...
				<?php } ?>
				<?php if ( ! empty( $invalid_expiration_items ) ) { ?>
					<p>
					<?php
						printf(
							/* translators: %s: comma-separated list of level names with links */
							wp_kses( __( '<strong>Warning:</strong> The following membership levels have an expiration date pattern that could not be understood and will be ignored: %s.', 'paid-memberships-pro' ), $expiration_warning_allowed_html ),
							implode( ', ', $invalid_expiration_items ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						);
					?>
					</p>
				<?php } ?>
