...
if ( $gateway_environment == 'sandbox' ) {
	$pmpro_reports['sales'] = __( 'Sales and Revenue (Testing/Sandbox)', 'pmpro' ); } else {
...
<?php
	...
	*/
	...
	function pmpro_getSales( $period, $levels = null ) {
		...
		if ( $period == 'today' ) {
			$startdate = date_i18n( 'Y-m-d', current_time( 'timestamp' ) ); } elseif ( $period == 'this month' ) {
			$startdate = date_i18n( 'Y-m', current_time( 'timestamp' ) ) . '-01'; } elseif ( $period == 'this year' ) {
				$startdate = date_i18n( 'Y', current_time( 'timestamp' ) ) . '-01-01'; } else {
				$startdate = ''; }
				...
				if ( ! empty( $cache ) && ! empty( $cache[ $period ] ) ) {
					$cache[ $period ][ $levels ] = $sales; } elseif ( ! empty( $cache ) ) {
					$cache[ $period ] = array( $levels => $sales ); } else {
	...
	function pmpro_getRevenue( $period, $levels = null ) {
		...
		if ( $period == 'today' ) {
			...
			$startdate = date_i18n( 'Y-m', current_time( 'timestamp' ) ) . '-01'; } elseif ( $period == 'this year' ) {
				...
				//save in cache
				if ( ! empty( $cache ) && ! empty( $cache[ $period ] ) ) {
					$cache[ $period ][ $levels ] = $revenue; } elseif ( ! empty( $cache ) ) {
					$cache[ $period ] = array( $levels => $revenue ); } else {
