...
function pmpro_report_memberships_page() {
	...
	if ( isset( $_REQUEST['type'] ) ) {
		$type = sanitize_text_field( $_REQUEST['type'] ); } else {
		$type = 'signup_v_all'; }

		if ( isset( $_REQUEST['period'] ) ) {
			$period = sanitize_text_field( $_REQUEST['period'] ); } else {
			$period = 'monthly'; }

			if ( isset( $_REQUEST['month'] ) ) {
				$month = intval( $_REQUEST['month'] ); } else {
				$month = date_i18n( 'n' ); }

				$thisyear = date_i18n( 'Y' );
				if ( isset( $_REQUEST['year'] ) ) {
					$year = intval( $_REQUEST['year'] ); } else {
					$year = date_i18n( 'Y' ); }

					if ( isset( $_REQUEST['level'] ) ) {
						$l = intval( $_REQUEST['level'] ); } else {
						$l = ''; }
		...
		FROM $wpdb->pmpro_memberships_users mu1 ";
							if ( $type === 'signup_v_cancel' ) {
								$sqlQuery .= "WHERE mu1.status IN('inactive','cancelled','cancelled_admin') "; } elseif ( $type === 'signup_v_expiration' ) {
								$sqlQuery .= "WHERE mu1.status IN('expired') "; } else {
		 ...
		 */
								...
								foreach ( $dates as $day => &$date ) {
									if ( ! empty( $cdates ) && ! empty( $cdates[ $day ] ) ) {
										$date->cancellations = $cdates[ $day ]->cancellations; } else {
		...
		function drawChart() {          
			...
			<?php if ( $type === 'signup_v_all' ) : // Signups vs. all cancellations ?>
			  ['<?php echo $date_function;?>', 'Signups', 'All Cancellations'],
				...
				['<?php if ( $period == 'monthly' ) { echo date_i18n( 'M', mktime( 0,0,0,$value->date,2 ) ); } elseif ( $period == 'daily' ) { echo $key; } else { echo $value->date; }?>', <?php echo $value->signups; ?>, <?php echo $value->cancellations; ?>],
				<?php } ?>
			<?php endif; ?>
			...
			<?php if ( $type === 'mrr_ltv' ) : ?>
			  ['<?php echo $date_function;?>', 'MRR', 'LTV'],
				<?php foreach ( $dates as $key => $value ) { ?>
				['<?php if ( $period == 'monthly' ) { echo date_i18n( 'M', mktime( 0,0,0,$value->date,2 ) ); } elseif ( $period == 'daily' ) { echo $key; } else { echo $value->date; }?>', <?php echo (($mrr = $value->total / $value->months) && $mrr != 0) ? $mrr : 0; ?>, <?php echo pmpro_getLTV( $period, null, $mrr ); ?>],
				<?php } ?>
			<?php endif; ?>
			]);
...
function pmpro_getSignups( $period = false, $levels = 'all' ) {
	...
	if ( $period == 'today' ) {
		$startdate = date_i18n( ' Y-m-d' ); } elseif ( $period == 'this month' ) {
		$startdate = date_i18n( 'Y-m' ) . '-01'; } elseif ( $period == 'this year' ) {
			$startdate = date_i18n( 'Y' ) . '-01-01'; } else {
			$startdate = ''; }
			...
			if ( ! empty( $cache ) && ! empty( $cache[ $period ] ) ) {
				$cache[ $period ][ $levels ] = $signups; } elseif ( ! empty( $cache ) ) {
				$cache[ $period ] = array( $levels => $signups ); } else {
...
function pmpro_getCancellations( $period = null, $levels = 'all', $status = array( 'inactive', 'expired', 'cancelled', 'admin_cancelled' ) ) {
	...
	*/
	...
	//save in cache
	if ( ! empty( $cache ) && ! empty( $cache[ $hash ] ) ) {
		$cache[ $hash ] = $cancellations; } elseif ( ! empty( $cache ) ) {
		$cache[ $hash ] = $cancellations; } else {
...
function pmpro_getMRR( $period, $levels = 'all' ) {
	...
	if ( $period == 'this month' ) {
		...
		*/
		...
		if ( $months > 0 ) {
			$mrr = $revenue / $months; } else {
			$mrr = 0; }

			//save in cache
			if ( ! empty( $cache ) && ! empty( $cache[ $period ] ) ) {
				$cache[ $period ][ $levels ] = $mrr; } elseif ( ! empty( $cache ) ) {
				$cache[ $period ] = array( $levels => $mrr ); } else {
...
function pmpro_getCancellationRate( $period, $levels = 'all', $status = null ) {
	...
	//save in cache
	if ( ! empty( $cache ) && ! empty( $cache[ $period ] ) ) {
		$cache[ $period ][ $levels ] = $rate; } elseif ( ! empty( $cache ) ) {
		$cache[ $period ] = array( $levels => $rate ); } else {
...
function pmpro_getLTV( $period, $levels = 'all', $mrr = null, $signups = null, $cancellation_rate = null ) {
	...
	if ( $signups > 0 ) {
		$ams = $mrr / $signups; } else {
		$ams = 0; }

		if ( $cancellation_rate > 0 ) {
			$ltv = $ams * (1 / $cancellation_rate); } else {
			$ltv = $ams; }
