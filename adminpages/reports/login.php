	...
	if ( isset( $_REQUEST['pn'] ) ) {
		...
		if ( isset( $_REQUEST['limit'] ) ) {
			...
			if ( $s ) {
				...
				if ( $l == 'all' ) {
					$sqlQuery .= " AND mu.status = 'active' AND mu.membership_id > 0 "; } elseif ( $l ) {
				...
				if ( $l == 'all' ) {
					$sqlQuery .= " AND mu.membership_id > 0  AND mu.status = 'active' "; } elseif ( $l ) {
					$sqlQuery .= " AND mu.membership_id = '" . $l . "' "; }
					$sqlQuery .= "GROUP BY u.ID ORDER BY user_registered DESC LIMIT $start, $limit";
			}
					...
					<td>
						...
						<strong>
							...
							<td>
								<?php
								if ( $auser->enddate ) {
									echo date_i18n( get_option( 'date_format' ), $auser->enddate ); } else {
									echo 'Never'; }
								?>
							</td>
