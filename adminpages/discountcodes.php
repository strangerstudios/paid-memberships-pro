								...
								<?php
									...
									foreach( $levels as $level ) {
										if ( ! empty( $pmpro_pages['checkout'] ) ) {
											$level_names[] = '<a title="' . pmpro_url( 'checkout', '?pmpro_level=' . $level->id . '&pmpro_discount_code=' . $code->code) . '" target="_blank" href="' . pmpro_url( 'checkout', '?pmpro_level=' . $level->id . '&pmpro_discount_code=' . $code->code) . '">' . $level->name . '</a>';
										} else {
