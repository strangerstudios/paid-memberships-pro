	...
	class PMProGateway_braintree extends PMProGateway
	{
      ...
      $plans = wp_cache_get( $cache_key,'pmpro_levels' );
		...
		static function pmpro_checkout_order($morder)
		{
			...
			if(isset($_REQUEST['number']))
				$braintree_number = sanitize_text_field( wp_unslash( $_REQUEST['number'] ) );
			else
				$braintree_number = "";

			if(isset($_REQUEST['expiration_date']))
				$braintree_expiration_date = sanitize_text_field( wp_unslash( $_REQUEST['expiration_date'] ) );
			else
				$braintree_expiration_date = "";

			if(isset($_REQUEST['cvv']))
				$braintree_cvv = sanitize_text_field( wp_unslash( $_REQUEST['cvv'] ) );
			else
				$braintree_cvv = "";
								...
								<?php
									...
									if($pmpro_show_cvv) { ?>
										<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_payment-cvv', 'pmpro_payment-cvv' ) ); ?>">
											...
											<input id="CVV" name="cvv" type="text" size="4" value="<?php if(!empty( $_REQUEST['CVV'])) { echo esc_attr( sanitize_text_field( wp_unslash( $_REQUEST['CVV'] ) ) ); } // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only. Prefills the checkout form. ?>" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_input pmpro_form_input-text', 'CVV' ) ); ?>" data-encrypted-name="cvv" />
										</div>
								<?php } ?>
      ...
      *
		...
		function cancel(&$order)
		{
			...
			if ( isset( $_POST['bt_signature']) && isset( $_POST['bt_payload']) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Braintree webhook payloads cannot carry a WordPress nonce; they are authenticated by Braintree_WebhookNotification::parse() verifying bt_signature.

				try {
					// Note: Braintree needs the raw data.
					$webhookNotification = Braintree_WebhookNotification::parse( $_POST['bt_signature'], $_POST['bt_payload'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- Braintree webhook payloads cannot carry a WordPress nonce; they are authenticated by Braintree_WebhookNotification::parse() verifying bt_signature. The signature and payload must be passed byte-for-byte as received.
