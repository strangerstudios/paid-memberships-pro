...
class PMProGateway_braintree extends PMProGateway {
	...
	public static function dependencies() {
		...
		foreach ( $modules as $module ) {
			if ( ! extension_loaded( $module ) ) {
				...
				$msg = -1;
				$msgt = sprintf( __( "The %1$s gateway depends on the %1$s PHP extension. Please enable it, or ask your hosting provider to enable it.", 'pmpro' ), 'Braintree', $module );
	...
	static function pmpro_checkout_order( $morder ) {
		...
		if ( isset( $_REQUEST['number'] ) ) {
			$braintree_number = $_REQUEST['number']; } else { 			$braintree_number = ''; }

		if ( isset( $_REQUEST['expiration_date'] ) ) {
			$braintree_expiration_date = $_REQUEST['expiration_date']; } else { 			$braintree_expiration_date = ''; }

		if ( isset( $_REQUEST['cvv'] ) ) {
			$braintree_cvv = $_REQUEST['cvv']; } else { 			$braintree_cvv = ''; }
	...
	*/
	function getCustomer( &$order, $force = false ) {
		...
		if ( ! empty( $order->accountnumber ) ) {
			...
			} else { 				update_user_meta( $user_id, 'pmpro_braintree_customerid', $this->customer->id ); }
