...
class PMProGateway_cybersource extends PMProGateway {
	...
	function getCardType( $name ) {
		...
		if ( isset( $card_types[ $name ] ) ) {
			return $card_types[ $name ]; } else { 			return false; }
	}

	function getWSDL( $order ) {
		//which gateway environment?
		if ( empty( $order->gateway_environment ) ) {
			$gateway_environment = pmpro_getOption( 'gateway_environment' ); } else { 			$gateway_environment = $order->gateway_environment; }

		//which host?
		if ( $gateway_environment == 'live' ) {
				$host = 'ics2ws.ic3.com'; } else { 				$host = 'ics2wstest.ic3.com'; }
		...
		*/
		...
		if ( $order->BillingPeriod == 'Year' ) {
			...
			if ( $order->BillingFrequency == 6 ) {
				$frequency = 'semi annually'; } elseif ( $order->BillingFrequency == 3 ) {
				$frequency = 'quarterly'; } else { 				$frequency = 'monthly'; }
		} elseif ( $order->BillingPeriod == 'Week' ) {
			if ( $order->BillingFrequency == 4 ) {
				$frequency = 'quad-weekly'; } elseif ( $order->BillingFrequency == 2 ) {
				$frequency = 'bi-weekly'; } else { 				$frequency = 'weekly'; }
		} elseif ( $order->BillingPeriod == 'Day' ) {
			if ( $order->BillingFrequency == 365 ) {
				$frequency = 'annually'; } elseif ( $order->BillingFrequency == 182 ) {
				$frequency = 'semi annually'; } elseif ( $order->BillingFrequency == 183 ) {
					$frequency = 'semi annually'; } elseif ( $order->BillingFrequency == 90 ) {
					$frequency = 'quaterly'; } elseif ( $order->BillingFrequency == 30 ) {
						$frequency = 'monthly'; } elseif ( $order->BillingFrequency == 15 ) {
						$frequency = 'semi-monthly'; } elseif ( $order->BillingFrequency == 28 ) {
							$frequency = 'quad-weekly'; } elseif ( $order->BillingFrequency == 14 ) {
							$frequency = 'bi-weekly'; } elseif ( $order->BillingFrequency == 7 ) {
								$frequency = 'weekly'; }
		}
	...
	function getErrorFromCode( $code ) {
		...
		if ( isset( $error_messages[ $code ] ) ) {
			return $error_messages[ $code ]; } else { 			return 'Unknown error.'; }
	}
}
