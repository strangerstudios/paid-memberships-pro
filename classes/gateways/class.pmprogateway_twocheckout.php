	...
	function sendToTwocheckout( &$order ) {
		...
		if ( pmpro_isLevelRecurring( $order->membership_level ) ) {
			...
			if ( property_exists( $order, 'TotalBillingCycles' ) ) {
				$tco_args['li_0_duration'] = ($order->BillingFrequency * $order->TotalBillingCycles ) . ' ' . $order->BillingPeriod; } else { 				$tco_args['li_0_duration'] = 'Forever'; }
		...
		if ( 'sandbox' === $gateway_environment || 'beta-sandbox' === $gateway_environment ) {
			...
			$tco_args['demo'] = 'Y';
		} else { 			Twocheckout::sandbox( false ); }
		...
		if ( $gateway_environment == 'live' ) {
				$host = 'www.2checkout.com'; } else { 				$host = 'sandbox.2checkout.com'; }
