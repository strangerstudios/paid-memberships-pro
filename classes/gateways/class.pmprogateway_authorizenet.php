...
class PMProGateway_authorizenet extends PMProGateway {
	...
	function authorize( &$order ) {
		...
		if ( $gateway_environment == 'live' ) {
				$host = 'secure.authorize.net'; } else { 				$host = 'test.authorize.net'; }
	...
	function subscribe( &$order ) {
		...
		if ( empty( $gateway_environment ) ) {
			$gateway_environment = pmpro_getOption( 'gateway_environment' ); }
		if ( $gateway_environment == 'live' ) {
				$host = 'api.authorize.net'; } else { 				$host = 'apitest.authorize.net'; }
		...
		if ( $order->BillingPeriod == 'Month' ) {
			$unit = 'months'; } elseif ( $order->BillingPeriod == 'Day' ) {
			$unit = 'days'; } elseif ( $order->BillingPeriod == 'Year' && $order->BillingFrequency == 1 ) {
				$unit = 'months';
				$length = 12;
			} elseif ( $order->BillingPeriod == 'Week' ) {
				$unit = 'days';
				$length = $length * 7;	//converting weeks to days
			} else { 			return false;	//authorize.net only supports months and days
			}
			...
			if ( isset( $order->TrialBillingCycles ) ) {
				$trialOccurrences = (int) $order->TrialBillingCycles; } else { 			$trialOccurrences = 0; }
			if ( isset( $order->TrialAmount ) ) {
				$trialAmount = $order->TrialAmount; } else { 			$trialAmount = null; }
			...
			if ( strpos( $order->billing->phone, '+' ) === false ) {
				$customer_phone = $order->billing->phone; } else { 			$customer_phone = ''; }
	...
	function cancel( &$order ) {
		...
		if ( ! empty( $order->subscription_transaction_id ) ) {
			$subscriptionId = $order->subscription_transaction_id; } else { 			$subscriptionId = ''; }
		$loginname = pmpro_getOption( 'loginname' );
		$transactionkey = pmpro_getOption( 'transactionkey' );

		if ( ! empty( $order->gateway_environment ) ) {
			$gateway_environment = $order->gateway_environment; } else { 			$gateway_environment = pmpro_getOption( 'gateway_environment' ); }
