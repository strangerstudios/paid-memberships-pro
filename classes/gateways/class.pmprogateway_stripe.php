	...
	public static function process_refund( $success, $order ) {
		...
		try {
			...
			if ( $refund->status != 'failed' ) {
				...
				$order->add_order_note( sprintf( __('Admin: Order successfully refunded on %1$s for transaction ID %2$s by %3$s.', 'paid-memberships-pro' ), date_i18n('Y-m-d H:i:s'), $transaction_id, $current_user->display_name ) );
