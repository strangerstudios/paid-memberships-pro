	...
	public static function dependencies() {
		...
		foreach ( $modules as $module ) {
			if ( ! extension_loaded( $module ) ) {
				...
				$msg = -1;
				$msgt = sprintf( __( "The %1$s gateway depends on the %1$s PHP extension. Please enable it, or ask your hosting provider to enable it.", 'pmpro' ), 'Stripe', $module );
	...
	static function user_profile_fields( $user ) {
		...
		if ( empty( $sub ) ) {
			...
			if ( empty( $_REQUEST['user_id'] ) ) {
				_e( "Subscription updates, allow you to change the member's subscription values at predefined times. Be sure to click Update Profile after making changes.", 'pmpro' ); } else { 					_e( "Subscription updates, allow you to change the member's subscription values at predefined times. Be sure to click Update User after making changes.", 'pmpro' ); }
			?>
			</p>
			...
			<tr>
				...
				<td id="updates_td">
					...
					} else { 							$updates = array( array( 'template' => true, 'when' => 'now', 'date_month' => '', 'date_day' => '', 'date_year' => '', 'billing_amount' => '', 'cycle_number' => '', 'cycle_period' => 'Month' ) ); }
		...
		for ( $i = 1; $i < count( $_POST['updates_when'] ); $i++ ) {
			...
			if ( $update['when'] == 'now' ) {
				...
				if ( ! empty( $next_on_date_update ) ) {
					$next_on_date_update = min( $next_on_date_update, $update['date_year'] . '-' . $update['date_month'] . '-' . $update['date_day'] ); } else { 					$next_on_date_update = $update['date_year'] . '-' . $update['date_month'] . '-' . $update['date_day']; }
			}
					...
					foreach ( $user_updates as $key => $ud ) {
						if ( $ud['when'] == 'date' &&
						   $ud['date_year'] . '-' . $ud['date_month'] . '-' . $ud['date_day'] <= date_i18n( 'Y-m-d', current_time( 'timestamp' ) )
							...
							if ( ! empty( $next_on_date_update ) ) {
								$next_on_date_update = min( $next_on_date_update, $ud['date_year'] . '-' . $ud['date_month'] . '-' . $ud['date_day'] ); } else { 								$next_on_date_update = $ud['date_year'] . '-' . $ud['date_month'] . '-' . $ud['date_day']; }
						}
					}
				}
		...
		if ( ! empty( $order->subscription_transaction_id ) && strpos( $order->subscription_transaction_id, 'cus_' ) !== false ) {
			...
			if ( empty( $customer_id ) && ! empty( $user_id ) ) {
				...
				if ( ! empty( $order->payment_transaction_id ) ) {
					$payment_transaction_id = $order->payment_transaction_id; } else {
					...
					if ( ! empty( $payment_transaction_id ) ) {
						...
						} elseif ( strpos( $payment_transaction_id, 'in_' ) !== false ) {
							//invoice look it up
			...
			if ( ! empty( $order->FirstName ) && ! empty( $order->LastName ) ) {
				$name = trim( $order->FirstName . ' ' . $order->LastName ); } elseif ( ! empty( $order->FirstName ) ) {
				$name = $order->FirstName; } elseif ( ! empty( $order->LastName ) ) {
					$name = $order->LastName; }
	...
	function subscribe( &$order, $checkout = true ) {
		...
		if ( ! empty( $order->user_id ) ) {
			...
			*/
			...
			} else { 			$trial_period_days = $order->BillingFrequency * 30;	//assume monthly
			}
			...
			if ( ! empty( $order->TrialBillingCycles ) && $order->TrialAmount == 0 ) {
				...
				} else { 				$trial_period_days = $trial_period_days + (30 * $order->BillingFrequency * $trialOccurrences);	//assume monthly
				}
			...
			if ( $checkout ) {
				...
				//update user meta
				if ( ! empty( $user_id ) ) {
					update_user_meta( $user_id, 'pmpro_stripe_updates', $new_user_updates ); } else {
		...
		if ( ! empty( $user_id ) ) {
			...
			if ( ! empty( $order->id ) && ! empty( $order->subscription_transaction_id ) && $order->gateway == 'stripe' ) {
				...
				if ( ! empty( $subscription->current_period_end ) ) {
					return $subscription->current_period_end; } else { 					return false; }
			}
		}
