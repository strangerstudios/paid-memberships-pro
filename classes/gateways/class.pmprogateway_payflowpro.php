...
class PMProGateway_payflowpro extends PMProGateway {
	...
	function authorize( &$order ) {
		...
		if ( $order->cardtype == 'American Express' ) {
			$cardtype = 'Amex'; } else { 			$cardtype = $order->cardtype; }
	...
	function subscribe( &$order ) {
		...
		if ( $order->BillingPeriod == 'Day' ) {
			$payperiod = 'DAYS'; } elseif ( $order->BillingPeriod == 'Week' ) {
			$payperiod = 'WEEK'; } elseif ( $order->BillingPeriod == 'Month' ) {
				$payperiod = 'MONT'; } elseif ( $order->BillingPeriod == 'Year' ) {
				$payperiod = 'YEAR'; }
				...
				if ( ! empty( $order->TotalBillingCycles ) ) {
					$nvpStr .= '&TERM=' . $order->TotalBillingCycles; } else { 			$nvpStr .= '&TERM=0'; }
