...
class PMProGateway_paypalstandard extends PMProGateway {
	...
	function sendToPayPal( &$order ) {
		...
		if ( 'sandbox' === $environment || 'beta-sandbox' === $environment ) {
			$paypal_url = 'https://www.sandbox.paypal.com/cgi-bin/webscr?business=' . urlencode( pmpro_getOption( 'gateway_email' ) ); } else { 			$paypal_url = 'https://www.paypal.com/cgi-bin/webscr?business=' . urlencode( pmpro_getOption( 'gateway_email' ) ); }

		if ( pmpro_isLevelRecurring( $order->membership_level ) ) {
			//convert billing period
			if ( $order->BillingPeriod == 'Day' ) {
				$period = 'D'; } elseif ( $order->BillingPeriod == 'Week' ) {
				$period = 'W'; } elseif ( $order->BillingPeriod == 'Month' ) {
					$period = 'M'; } elseif ( $order->BillingPeriod == 'Year' ) {
					$period = 'Y'; } else {
					...
					*/
					...
					if ( ! empty( $order->TotalBillingCycles ) ) {
						...
						if ( $srt > 1 ) {
							$paypal_args['srt'] = $srt; } else { 					$paypal_args['src'] = '0'; }
					} else { 				$paypal_args['srt'] = '0';	//indefinite subscription
					}
		} else {
			//other args
			$paypal_args = array(
