...
class PMProGateway_paypalexpress extends PMProGateway {
	...
	static function pmpro_checkout_before_processing() {
		...
		if ( ! $current_user->ID ) {
			...
			if ( isset( $_REQUEST['username'] ) ) {
				$username = trim( $_REQUEST['username'] ); } else { 				$username = ''; }
			if ( isset( $_REQUEST['password'] ) ) {
				$password = $_REQUEST['password']; } else { 				$password = ''; }
			if ( isset( $_REQUEST['bemail'] ) ) {
				$bemail = $_REQUEST['bemail']; } else { 				$bemail = ''; }
	...
	static function pmpro_checkout_confirmed( $pmpro_confirmed ) {
		...
		if ( ! empty( $morder ) ) {
			return array( 'pmpro_confirmed' => $pmpro_confirmed, 'morder' => $morder ); } else { 			return $pmpro_confirmed; }
	}
