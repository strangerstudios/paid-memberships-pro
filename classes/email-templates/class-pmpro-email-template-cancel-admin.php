...
<p>Log in to your WordPress admin here: {{ login_url }}</p>', 'paid-memberships-pro' ) );
	...
	public function get_email_template_variables() {
		...
		if ( empty( $this->cancelled_level_ids ) ) {
			...
			$email_template_variables['membership_level_name'] = pmpro_implodeToEnglish( $wpdb->get_col( "SELECT name FROM $wpdb->pmpro_membership_levels WHERE id IN('" . implode( "','", $this->cancelled_level_ids ) . "')" ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Level IDs are integers from core callers (intval'd request values or level IDs loaded from the database).
		} else {
			...
			$email_template_variables['membership_level_name'] = pmpro_implodeToEnglish( $wpdb->get_col( "SELECT name FROM $wpdb->pmpro_membership_levels WHERE id = '" . $this->cancelled_level_ids . "'" ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Level IDs are integers from core callers (intval'd request values or level IDs loaded from the database).
		}
	...
	private function get_start_and_end_date( $end_or_start_date ) {
		...
		// phpcs:disable WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- $end_or_start_date is only ever the literal startdate or enddate, the user ID comes from a WP_User object, and the level ID is an integer from core callers.
		return $wpdb->get_var(
