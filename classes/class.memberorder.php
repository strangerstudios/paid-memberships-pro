...
class MemberOrder {
	...
	function __construct( $id = null ) {
		...
		if ( $id ) {
			if ( is_numeric( $id ) ) {
				return $this->getMemberOrderByID( $id ); } else { 				return $this->getMemberOrderByCode( $id ); }
		} else { 			return $this->getEmptyMemberOrder();	//blank constructor
		}	}
	...
	function getMemberOrderByID( $id ) {
		...
		if ( $dbobj ) {
			...
			if ( ! empty( $nameparts['fname'] ) ) {
				$this->FirstName = $nameparts['fname']; } else { 				$this->FirstName = ''; }
			if ( ! empty( $nameparts['lname'] ) ) {
				$this->LastName = $nameparts['lname']; } else { 				$this->LastName = ''; }
			...
			return $this->id;
		} else { 			return false;	//didn't find it in the DB
		}	}
	...
	function getLastMemberOrder( $user_id = null, $status = 'success', $membership_id = null, $gateway = null, $gateway_environment = null ) {
		...
		if ( ! empty( $status ) && is_array( $status ) ) {
			$this->sqlQuery .= "AND status IN('" . implode( "','", $status ) . "') "; } elseif ( ! empty( $status ) ) {
	...
	*/
	function getMemberOrderByCode( $code ) {
		...
		if ( $id ) {
			return $this->getMemberOrderByID( $id ); } else { 			return false; }
	}
	...
	*/
	...
	function getMembershipLevel( $force = false ) {
		...
		if ( ! empty( $this->discount_code ) && empty( $this->membership_level->membership_id ) ) {
			if ( ! empty( $this->discount_code->code ) ) {
				$discount_code = $this->discount_code->code; } else { 				$discount_code = $this->discount_code; }
	...
	function updateTimestamp( $year, $month, $day, $time = null ) {
		...
		if ( $wpdb->query( $this->sqlQuery ) !== 'false' ) {
			return $this->getMemberOrderByID( $this->id ); } else { 			return false; }
	}
	...
	function saveOrder() {
		...
		if ( ! empty( $this->InitialPayment ) ) {
			$amount = $this->InitialPayment; } elseif ( ! empty( $this->subtotal ) ) {
			$amount = $this->subtotal; } else { 			$amount = 0; }

			//Todo: Tax?!, Coupons, Certificates, affiliates
			if ( empty( $this->subtotal ) ) {
				$this->subtotal = $amount; }
			if ( isset( $this->tax ) ) {
				$tax = $this->tax; } else { 			$tax = $this->getTax( true ); }
			$this->certificate_id = '';
			$this->certificateamount = '';

			//calculate total
			if ( ! empty( $this->total ) ) {
				$total = $this->total; } else {
