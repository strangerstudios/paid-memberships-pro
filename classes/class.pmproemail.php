...
class PMProEmail {
	...
	function sendCancelAdminEmail( $user = null, $old_level_id = null ) {
		...
		$this->subject = sprintf( __( "Membership for %1$s at %1$s has been CANCELLED", 'pmpro' ), $user->user_login, get_option( 'blogname' ) );
		...
		if ( ! empty( $old_level_id ) ) {
			...
			if ( ! empty( $startdate ) ) {
				$this->data['startdate'] = date_i18n( get_option( 'date_format' ), $startdate ); } else { 				$this->data['startdate'] = ''; }
			...
			if ( ! empty( $enddate ) ) {
				$this->data['enddate'] = date_i18n( get_option( 'date_format' ), $enddate ); } else { 				$this->data['enddate'] = ''; }
		} else {
			$this->data['membership_id'] = '';
			$this->data['membership_level_name'] = __( 'All Levels', 'pmpro' );
			$this->data['startdate'] = '';
	...
	function sendCheckoutEmail( $user = null, $invoice = null ) {
		...
		if ( ! empty( $invoice ) && ! pmpro_isLevelFree( $user->membership_level ) ) {
			if ( $invoice->gateway == 'paypalexpress' ) {
				$this->template = 'checkout_express'; } elseif ( $invoice->gateway == 'check' ) {
				...
				$this->data['instructions'] = wpautop( pmpro_getOption( 'instructions' ) );
					...
					$this->template = 'checkout_trial'; } else { 				$this->template = 'checkout_paid'; }
	...
	function sendCheckoutAdminEmail( $user = null, $invoice = null ) {
		...
		$this->email = get_bloginfo( 'admin_email' );
		$this->subject = sprintf( __( "Member Checkout for %1$s at %1$s", 'pmpro' ), $user->membership_level->name, get_option( 'blogname' ) );
		...
		if ( ! empty( $invoice ) && ! pmpro_isLevelFree( $user->membership_level ) ) {
			if ( $invoice->gateway == 'paypalexpress' ) {
				$this->template = 'checkout_express_admin'; } elseif ( $invoice->gateway == 'check' ) {
				$this->template = 'checkout_check_admin'; } elseif ( pmpro_isLevelTrial( $user->membership_level ) ) {
					$this->template = 'checkout_trial_admin'; } else { 				$this->template = 'checkout_paid_admin'; }
				$this->data['invoice_id'] = $invoice->code;
				$this->data['invoice_total'] = pmpro_formatPrice( $invoice->total );
				$this->data['invoice_date'] = date_i18n( get_option( 'date_format' ), $invoice->timestamp );
				$this->data['billing_name'] = $invoice->billing->name;
	...
	function sendBillingAdminEmail( $user = null, $invoice = null ) {
		...
		$this->email = get_bloginfo( 'admin_email' );
		$this->subject = sprintf( __( "Billing information has been updated for %1$s at %1$s", 'pmpro' ), $user->user_login, get_option( 'blogname' ) );
	...
	function sendBillingFailureAdminEmail( $email, $invoice = null ) {
		...
		$this->email = $email;
		$this->subject = sprintf( __( "Membership Payment Failed For %1$s at %1$s", 'pmpro' ), $user->display_name, get_option( 'blogname' ) );
	...
	function sendAdminChangeEmail( $user = null ) {
		...
		if ( $user->membership_level->ID ) {
			$this->data['membership_change'] = sprintf( __( 'The new level is %s', 'pmpro' ), $user->membership_level->name ); } else { 			$this->data['membership_change'] = __( 'Your membership has been cancelled', 'pmpro' ); }
	...
	function sendAdminChangeAdminEmail( $user = null ) {
		...
		$this->email = get_bloginfo( 'admin_email' );
		$this->subject = sprintf( __( "Membership for %1$s at %1$s has been changed", 'pmpro' ), $user->user_login, get_option( 'blogname' ) );
		...
		if ( $user->membership_level->ID ) {
			$this->data['membership_change'] = sprintf( __( 'The new level is %s', 'pmpro' ), $user->membership_level->name ); } else { 			$this->data['membership_change'] = __( 'Membership has been cancelled', 'pmpro' ); }
