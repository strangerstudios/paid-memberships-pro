	...
	class PMProEmail
	{
		...
		function sendCancelEmail($user = NULL, $old_level_id = NULL)
		{
			...
			if(!empty($old_level_id)) {
				if(!is_array($old_level_id))
					$old_level_id = array_map( 'esc_sql', array($old_level_id) );
		...
		function sendCancelAdminEmail($user = NULL, $old_level_id = NULL)
		{
			...
			if(!empty($old_level_id)) {
				...
				//start and end date
				$startdate = $wpdb->get_var("SELECT UNIX_TIMESTAMP(CONVERT_TZ(startdate, '+00:00', @@global.time_zone)) as startdate FROM $wpdb->pmpro_memberships_users WHERE user_id = '" . esc_sql( $user->ID ) . "' AND membership_id = '" .  $old_level_id[0] . "' AND status IN('inactive', 'cancelled', 'admin_cancelled') ORDER BY id DESC"); // We escape $old_level_id array earlier in the function. No need to escape it again.
				if(!empty($startdate))
					$this->data['startdate'] = date_i18n(get_option('date_format'), $startdate);
				else
					$this->data['startdate'] = "";
				$enddate = $wpdb->get_var("SELECT UNIX_TIMESTAMP(CONVERT_TZ(enddate, '+00:00', @@global.time_zone)) as enddate FROM $wpdb->pmpro_memberships_users WHERE user_id = '" . esc_sql( $user->ID ) . "' AND membership_id = '" . $old_level_id[0] . "' AND status IN('inactive', 'cancelled', 'admin_cancelled') ORDER BY id DESC"); // We escape $old_level_id array earlier in the function. No need to escape it again.
				if(!empty($enddate))
		...
		function sendCheckoutEmail($user = NULL, $invoice = NULL)
		{
			...
			$enddate = $wpdb->get_var("SELECT UNIX_TIMESTAMP(CONVERT_TZ(enddate, '+00:00', @@global.time_zone)) FROM $wpdb->pmpro_memberships_users WHERE user_id = '" . esc_sql( $user->ID ) . "' AND status = 'active' LIMIT 1");
			if( $enddate ) {
