...
jQuery(document).ready(function ($) {
  ...
  if ("undefined" !== typeof pmproStripe.paymentIntent) {
    if ("requires_action" === pmproStripe.paymentIntent.status) {
      ...
      stripe
        .handleCardAction(pmproStripe.paymentIntent.client_secret)
  ...
  if ("undefined" !== typeof pmproStripe.setupIntent) {
    if ("requires_action" === pmproStripe.setupIntent.status) {
      ...
      $("#pmpro_processing_message").css("visibility", "visible");
      stripe
        .handleCardSetup(pmproStripe.setupIntent.client_secret)
        .then(pmpro_stripeResponseHandler);
    }
  }
  ...
  function pmpro_stripeResponseHandler(response) {
    ...
    if (response.error) {
      ...
      if ($("#CardType[name=CardType]").length) {
        ...
        form.append(
          '<input type="hidden" name="CardType" value="' + card.brand + '"/>'
        );
      }

      form.append(
        '<input type="hidden" name="AccountNumber" value="XXXXXXXXXXXX' +
          card.last4 +
          '"/>'
      );
      form.append(
      ...
      // success message
      $("#pmpro_message")
        .text(pmproStripe.msgAuthenticationValidated)
        .addClass("pmpro_success")
        .removeClass("pmpro_alert")
        .removeClass("pmpro_error")
        .show();
      ...
      if (pmproStripe.paymentIntent) {
        form.append(
          '<input type="hidden" name="payment_intent_id" value="' +
            pmproStripe.paymentIntent.id +
            '" />'
        );
      }
      if (pmproStripe.setupIntent) {
        form.append(
          '<input type="hidden" name="setup_intent_id" value="' +
            pmproStripe.setupIntent.id +
            '" />'
        );
        form.append(
          '<input type="hidden" name="subscription_id" value="' +
            pmproStripe.subscription.id +
            '" />'
        );
      }
