...
function pmpro_shortcode_membership( $atts, $content = null, $code = '' ) {
	...
	if ( $hasaccess && ! empty( $delay ) ) {
		...
		if ( ! empty( $levels ) ) {
			$sqlQuery = "SELECT UNIX_TIMESTAMP(startdate) FROM $wpdb->pmpro_memberships_users WHERE status = 'active' AND membership_id IN(" . implode( ',', $levels ) . ") AND user_id = '" . $current_user->ID . "' ORDER BY id LIMIT 1"; } else { 			$sqlQuery = "SELECT UNIX_TIMESTAMP(startdate) FROM $wpdb->pmpro_memberships_users WHERE status = 'active' AND user_id = '" . $current_user->ID . "' ORDER BY id LIMIT 1"; }
