...
function pmpro_twocheckoutExit( $redirect = false ) {
	...
	if ( defined( 'PMPRO_INS_DEBUG' ) && PMPRO_INS_DEBUG === 'log' ) {
		...
		} else { 			$log_email = get_option( 'admin_email' ); }
...
function pmpro_twocheckoutValidate() {
	...
	if ( empty( $params['message_type'] ) ) {
		$check = Twocheckout_Return::check( $params, pmpro_getOption( 'twocheckout_secretword' ), 'array' ); } else { 	$check = Twocheckout_Notification::check( $params, pmpro_getOption( 'twocheckout_secretword' ), 'array' ); }

	if ( empty( $check ) ) {
		$r = false;	//HTTP failure
	} elseif ( empty( $check['response_code'] ) ) {
		$r = false;	//Invalid response
	} else { 	$r = $check['response_code']; }
...
function pmpro_insChangeMembershipLevel( $txn_id, &$morder ) {
	...
	if ( pmpro_changeMembershipLevel( $custom_level, $morder->user_id ) !== false ) {
		...
		if ( $recurring ) {
			$morder->subscription_transaction_id = $txn_id; } else { 			$morder->subscription_transaction_id = ''; }
		$morder->saveOrder();
		...
		if ( ! empty( $morder ) ) {
			$invoice = new MemberOrder( $morder->id ); } else { 			$invoice = null; }
