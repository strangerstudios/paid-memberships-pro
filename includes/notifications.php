...
function pmpro_get_gocodebox_banner_notifier() {
	...
	// Check if the Gocodebox_Banner_Notifier class already exists.
	if ( class_exists( 'Gocodebox_Banner_Notifier' ) ) {
		// This must have already been loaded elsewhere. Log a potential library conflict.
		$previously_loaded_class = new \ReflectionClass( 'Gocodebox_Banner_Notifier' );
		pmpro_track_library_conflict( 'gocodebox_banner_notifer', $previously_loaded_class->getFileName(), 'unknown' );
	} else {
		// Include the Gocodebox_Banner_Notifier class file.
		require_once( PMPRO_DIR . '/includes/lib/notifications.php' );
	}

	// Create a new instance of the Gocodebox_Banner_Notifier class.
	$cached_notifier = new Gocodebox_Banner_Notifier(
		array(
			'prefix'            => 'pmpro',
...
add_action( 'admin_init', 'pmpro_get_gocodebox_banner_notifier' );
