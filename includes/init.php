...
function pmpro_init() {
	...
	if ( is_admin() ) {
		...
		if ( file_exists( get_stylesheet_directory() . '/paid-memberships-pro/css/print.css' ) ) {
			$print_css = get_stylesheet_directory_uri() . '/paid-memberships-pro/css/print.css'; } elseif ( file_exists( get_template_directory() . '/paid-memberships-pro/print.css' ) ) {
			$print_css = get_template_directory_uri() . '/paid-memberships-pro/print.css'; } else { 			$print_css = plugins_url( 'css/print.css',dirname( __FILE__ ) ); }
	...
	if ( ! empty( $pmpro_currencies[ $pmpro_currency ] ) && is_array( $pmpro_currencies[ $pmpro_currency ] ) ) {
		$pmpro_currency_symbol = $pmpro_currencies[ $pmpro_currency ]['symbol']; } elseif ( ! empty( $pmpro_currencies[ $pmpro_currency ] ) && strpos( $pmpro_currencies[ $pmpro_currency ], '(' ) !== false ) {
		$pmpro_currency_symbol = pmpro_getMatches( '/\((.*)\)/', $pmpro_currencies[ $pmpro_currency ], true ); } else { 		$pmpro_currency_symbol = $pmpro_currency . ' ';	//just use the code
		}}
...
function pmpro_manage_users_custom_column( $column_data, $column_name, $user_id ) {
	...
	if ( $column_name == 'pmpro_membership_level' ) {
		...
		} else { 			$column_data = __( 'None', 'pmpro' ); }
	}
	return $column_data;
}
