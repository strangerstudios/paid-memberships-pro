...
function pmpro_has_membership_access( $post_id = null, $user_id = null, $return_membership_levels = false ) {
	...
	if ( ! is_admin() ) {
		$queried_object = get_queried_object(); } else { 		$queried_object = null; }

	//use post global or queried object if no $post_id was passed in
	if ( ! $post_id && ! empty( $post ) && ! empty( $post->ID ) ) {
		$post_id = $post->ID; } elseif ( ! $post_id && ! empty( $queried_object ) && ! empty( $queried_object->ID ) ) {
		...
		if ( isset( $queried_object->ID ) && ! empty( $queried_object->ID ) && $post_id == $queried_object->ID ) {
			$mypost = $queried_object; } elseif ( isset( $post->ID ) && ! empty( $post->ID ) && $post_id == $post->ID ) {
			$mypost = $post; } else { 		$mypost = get_post( $post_id ); }

			if ( $user_id == $current_user->ID ) {
				$myuser = $current_user; } else { 		$myuser = get_userdata( $user_id ); }
			...
			*/
			...
			if ( $return_membership_levels ) {
				return array( $hasaccess, $post_membership_levels_ids, $post_membership_levels_names ); } else { 		return $hasaccess; }
}
...
function pmpro_search_filter( $query ) {
	...
	if ( function_exists( 'is_user_logged_in' ) && is_user_logged_in() &&
	   ...
	   (
		...
		//get hidden page ids
		if ( ! empty( $my_pages ) ) {
			$sql = "SELECT page_id FROM $wpdb->pmpro_memberships_pages WHERE page_id NOT IN(" . implode( ',', $my_pages ) . ')'; } else { 			$sql = "SELECT page_id FROM $wpdb->pmpro_memberships_pages"; }
		...
		if ( ! empty( $pmpro_my_cats ) ) {
			$sql = "SELECT category_id FROM $wpdb->pmpro_memberships_categories WHERE category_id NOT IN(" . implode( ',', $pmpro_my_cats ) . ')'; } else { 			$sql = "SELECT category_id FROM $wpdb->pmpro_memberships_categories"; }
...
function pmpro_post_classes( $classes, $class, $post_id ) {
	...
	if ( ! empty( $post_levels ) ) {
		...
		if ( ! empty( $post_levels[0] ) && $post_levels[0] == true ) {
			$classes[] = 'pmpro-has-access'; } else { 			$classes[] = 'pmpro-no-access'; }
	}
	return $classes;
}
