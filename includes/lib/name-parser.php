...
if ( ! function_exists( 'pnp_split_full_name' ) ) {
			...
			if ( pnp_is_initial( $word ) ) {
				...
				if ( $i == $start ) {
					...
					if ( pnp_is_initial( $name_parts[ $i + 1 ] ) ) {
						$fname .= ' ' . strtoupper( $word ); } else { 						$initials .= ' ' . strtoupper( $word ); }
	...
	function pnp_is_salutation( $word ) {
		...
		if ( $word == 'mr' || $word == 'master' || $word == 'mister' ) {
			return 'Mr.'; } elseif ( $word == 'mrs' ) {
			return 'Mrs.'; } elseif ( $word == 'miss' || $word == 'ms' ) {
				return 'Ms.'; } elseif ( $word == 'dr' ) {
				return 'Dr.'; } elseif ( $word == 'rev' ) {
					return 'Rev.'; } elseif ( $word == 'fr' ) {
					return 'Fr.'; } else { 			return false; }
	}
