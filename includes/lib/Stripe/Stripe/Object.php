...
class Stripe_Object implements ArrayAccess {
	...
	*/
	public function refreshFrom( $values, $apiKey, $partial = false ) {
		...
		if ( $partial ) {
			$removed = new Stripe_Util_Set(); } else { 		$removed = array_diff( array_keys( $this->_values ), array_keys( $values ) ); }
		...
		foreach ( $values as $k => $v ) {
			...
			if ( self::$nestedUpdatableAttributes->includes( $k ) && is_array( $v ) ) {
				$this->_values[ $k ] = Stripe_Object::scopedConstructFrom( 'Stripe_AttachedObject', $v, $apiKey ); } else { 			$this->_values[ $k ] = Stripe_Util::convertToStripeObject( $v, $apiKey ); }
	...
	*/
	...
	public function __toJSON() {
		...
		if ( defined( 'JSON_PRETTY_PRINT' ) ) {
			return json_encode( $this->__toArray( true ), JSON_PRETTY_PRINT ); } else { 		return json_encode( $this->__toArray( true ) ); }
	}
	...
	public function __toArray( $recursive = false ) {
		if ( $recursive ) {
			return Stripe_Util::convertStripeObjectToArray( $this->_values ); } else { 		return $this->_values; }
	}
}
