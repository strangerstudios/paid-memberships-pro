...
<table class="form-table">
		...
		foreach ( $levels as $level ) {
			...
			if ( empty( $membership_values ) || pmpro_isLevelFree( $membership_values ) ) {
				if ( ! empty( $membership_values->original_initial_payment ) && $membership_values->original_initial_payment > 0 ) {
					echo 'Paid ' . pmpro_formatPrice( $membership_values->original_initial_payment ) . '.'; } else { 					_e( 'Not paying.', 'pmpro' ); }
			} else {
	...
	if ( $show_expiration ) {
		...
		if ( $end_date ) {
			$selected_expires_day = date_i18n( 'j', $user->membership_level->enddate ); } else { 		$selected_expires_day = $current_day; }

		$current_month = date_i18n( 'M', current_time( 'timestamp' ) );
		if ( $end_date ) {
			$selected_expires_month = date_i18n( 'm', $user->membership_level->enddate ); } else { 		$selected_expires_month = date_i18n( 'm' ); }

		$current_year = date_i18n( 'Y', current_time( 'timestamp' ) );
		if ( $end_date ) {
			$selected_expires_year = date_i18n( 'Y', $user->membership_level->enddate ); } else { 		$selected_expires_year = (int) $current_year + 1; }
		?>
		<tr>
...
function pmpro_membership_level_profile_fields_update() {
	...
	if ( isset( $_REQUEST['membership_level'] ) ) {
		...
		} else { 			$changed_or_cancelled = 'admin_changed'; }
