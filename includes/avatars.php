...
function pmpro_display_avatar_field( $user_id, $markup ) {
	...
	if ( 'tr' === $markup ) {
		...
		echo '<div class="' . esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-file pmpro_form_field-avatar' ) ) . '">';
	}

	// Show the label.
	echo '<label for="pmpro_avatar" class="' . esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ) . '">' . esc_html__( 'Profile Picture', 'paid-memberships-pro' ) . '</label>';
