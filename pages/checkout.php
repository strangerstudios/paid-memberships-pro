	...
	if ( $include_pricing_fields ) {
		...
		<div id="pmpro_pricing_fields" class="pmpro_checkout">
			...
			if ( count( $pmpro_levels ) > 1 ) {
				?>
				<span class="pmpro_checkout-h3-msg"><a href="<?php echo pmpro_url( 'levels' ); ?>"><?php _e( 'change', 'paid-memberships-pro' ); ?></a></span><?php } ?>
			</h3>
...
?>
	...
	<?php
		include PMPRO_DIR . '/includes/card-cvv.php';
			...
			<div class="pmpro_checkout-fields">
				...
				if ( $pmpro_show_cvv ) {
					...
					<div class="pmpro_checkout-field pmpro_payment-cvv">
						...
						<input id="CVV" name="CVV" type="text" size="4" value="
						<?php
						if ( ! empty( $_REQUEST['CVV'] ) ) {
							echo esc_attr( $_REQUEST['CVV'] ); }
						?>
							" class="input <?php echo pmpro_getClassForField( 'CVV' ); ?>" />  <small>(<a href="#" id="credit-card-whats"><?php _e( "what's this?", 'paid-memberships-pro' ); ?></a>)</small>
					</div>
				<?php } ?>
				<?php if ( $pmpro_show_discount_code ) { ?>
