...
<tbody>
	...
	foreach ( $pmpro_levels as $level ) {
		if ( isset( $current_user->membership_level->ID ) ) {
			$current_level = ($current_user->membership_level->ID == $level->id); } else { 		  $current_level = false; }
	?>
	<tr class="<?php if ( $count++ % 2 == 0 ) { ?>odd<?php } ?><?php if ( $current_level == $level ) { ?> active<?php } ?>">
		...
		<td>
			...
			if ( pmpro_isLevelFree( $level ) ) {
				$cost_text = '<strong>' . __( 'Free', 'pmpro' ) . '</strong>'; } else { 					$cost_text = pmpro_getLevelCost( $level, true, true ); }
				$expiration_text = pmpro_getLevelExpiration( $level );
			if ( ! empty( $cost_text ) && ! empty( $expiration_text ) ) {
				echo $cost_text . '<br />' . $expiration_text; } elseif ( ! empty( $cost_text ) ) {
				echo $cost_text; } elseif ( ! empty( $expiration_text ) ) {
