...
if ( empty( $current_user->membership_level ) ) {
	$confirmation_message = '<p>' . __( 'Your payment has been submitted. Your membership will be activated shortly.', 'pmpro' ) . '</p>'; } else { 		$confirmation_message = '<p>' . sprintf( __( 'Thank you for your membership to %1$s. Your %1$s membership is now active.', 'pmpro' ), get_bloginfo( 'name' ), $current_user->membership_level->name ) . '</p>'; }
...
<?php if ( ! empty( $pmpro_invoice ) && ! empty( $pmpro_invoice->id ) ) { ?>		
	...
	<h3>
		<?php printf( __( 'Invoice #%1$s on %1$s', 'pmpro' ), $pmpro_invoice->code, date_i18n( get_option( 'date_format' ), $pmpro_invoice->timestamp ) );?>		
	</h3>
