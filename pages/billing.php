	...
	<?php if ( $level->billing_amount > 0 ) { ?>
			<li><strong><?php _e( 'Membership Fee', 'pmpro' );?>:</strong>
				...
				if ( $current_user->membership_level->cycle_number > 1 ) {
					printf( __( '%1$s every %2$d %1$s.', 'pmpro' ), pmpro_formatPrice( $level->billing_amount ), $level->cycle_number, pmpro_translate_billing_period( $level->cycle_period, $level->cycle_number ) );
				} elseif ( $current_user->membership_level->cycle_number == 1 ) {
					printf( __( '%1$s per %1$s.', 'pmpro' ), pmpro_formatPrice( $level->billing_amount ), pmpro_translate_billing_period( $level->cycle_period ) );
				} else {
