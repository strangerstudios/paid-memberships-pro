<?php
	...
	{	
		...
		$sqlQuery = "SELECT mu.user_id, mu.membership_id, mu.startdate, mu.enddate FROM $wpdb->pmpro_memberships_users mu LEFT JOIN $wpdb->usermeta um ON um.user_id = mu.user_id AND um.meta_key = 'pmpro_expiration_notice' WHERE mu.enddate IS NOT NULL AND DATE_SUB(enddate, INTERVAL 7 Day) <= '" . $today . "' AND (um.meta_value IS NULL OR DATE_ADD(meta_value, INTERVAL 7 Day) <= '" . $today . "') ORDER BY mu.enddate";
	...
	function pmpro_cron_expire_memberships()
	{
		...
		$sqlQuery = "SELECT mu.user_id, mu.membership_id, mu.startdate, mu.enddate FROM $wpdb->pmpro_memberships_users mu WHERE mu.enddate IS NOT NULL AND DATE(mu.enddate) <= '" . $today . "' ORDER BY mu.enddate";
	...
	function pmpro_cron_trial_ending_warnings()
	{
		...
		WHERE 
			...
			(
				(cycle_period = 'Day' AND DATE_ADD(startdate, INTERVAL trial_limit Day) <= DATE_ADD('" . $today . "', INTERVAL 7 Day)) OR
				(cycle_period = 'Week' AND DATE_ADD(startdate, INTERVAL trial_limit Week) <= DATE_ADD('" . $today . "', INTERVAL 7 Day)) OR
				(cycle_period = 'Month' AND DATE_ADD(startdate, INTERVAL trial_limit Month) <= DATE_ADD('" . $today . "', INTERVAL 7 Day)) OR
				(cycle_period = 'Year' AND DATE_ADD(startdate, INTERVAL trial_limit Year) <= DATE_ADD('" . $today . "', INTERVAL 7 Day)) 
			)			
						
			AND (um.meta_value IS NULL OR DATE_ADD(meta_value, INTERVAL 7 Day) <= '" . $today . "') 
